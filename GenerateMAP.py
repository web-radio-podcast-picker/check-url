...
def add_pulse_marker(map_obj, lat, lon, color, popup_html, strength):
    """Add a pulse-shimmer marker with size/speed based on density"""
...
def create_map_from_csv(output_file, map_file='output/map.html'):
    ...
    # Embedded audio player + info box
    audio_player_html = """
    <div style="position: fixed; bottom: 10px; left: 50%; transform: translateX(-50%);
        ...
        <div id="stationInfo" style="margin-top:5px; font-size:12px;"></div>
    </div>
    """
        ...
        for row in reader:
            ...
            if lat not in ['Unknown', '', None] and lon not in ['Unknown', '', None]:
                try:
                    ...
                    coord_groups[key].append((url, name, icyname, codec, sample_rate, bitrate))
                except ValueError:
                    continue
    ...
    for (lat, lon), url_list in coord_groups.items():
        ...
        # Scrollable popup with station names
        popup_html = f'<div style="max-height:200px; overflow-y:auto;"><b>Stations at this location: {count}</b><br>'
        for url, name, icyname, codec, sample_rate, bitrate in url_list:
            popup_html += (
                f'<a href="#" style="color:blue; text-decoration:underline; cursor:pointer;" '
                f'onclick="document.getElementById(\'stationPlayer\').src=\'{url}\'; '
                f'document.getElementById(\'stationPlayer\').play(); '
                f'document.getElementById(\'stationInfo\').innerHTML='
                f'\'<b>Name:</b> {name}<br><b>ICY Name:</b> {icyname}<br>'
                f'<b>Codec:</b> {codec}<br><b>Sample Rate:</b> {sample_rate}<br>'
                f'<b>Bitrate:</b> {bitrate}\'; '
                f'return false;">{name}</a><br>'
            )
        popup_html += '</div>'

        add_pulse_marker(m, lat, lon, color, popup_html, strength)
